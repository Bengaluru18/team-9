<html>
	<head>
		<title>Careworks Foundation</title>
		<meta name="viewport" content="width=device-width, initial-scale=1" />
		<link rel="stylesheet" href="assets/css/main.css" />
	</head>
	<body>
			<header id="header">
				<div class="inner">
					
					<a href="index.html" class="logo"><strong>A Non-profit Organisation</strong></a>
					<nav id="nav">
						<a href="index.html">Home</a>
						<a href="contactus.html">Contact Us</a>
					</nav>
					<a href="#navPanel" class="navPanelToggle"><span class="fa fa-bars"></span></a>
				</div>
			</header>

			<section id="banner">
				<div class="inner">
					<header>
						<img src="images/logo.png" alt="Pic 01" /><h1>Careworks Foundation</h1>
					</header>

					<div class="flex">

						<div>
							<span class="icon fa-car"></span>
							<h3>ACADEMIC SUPPORT  </h3>
							<p>The activities that are organised</p>
						</div>
					</div>

					<footer>
						<div class="button" class="button" onclick="openTab('b1')">Learn More</div>
					</footer>
				</div>
			</section>
			<div id="b1" class="containerTab" style="display:none;">
 		    <span onclick="this.parentElement.style.display='none'" class="closebtn">&times;</span>
  			<h2>Education</h2>
  			<p>
				Education is not a luxury, it’s a necessity. As a result of unavailability of financial resources, a lot of families cannot afford to spend on the basic things needed for their child’s education. Careworks Foundation strives to bridge this gap between the students and their education by overcoming such barriers and helping them attain their right to knowledge and quality education.

				School kit distribution is an annual activity that happens at the beginning of the academic year. The kit consists of school bag, note books, stationary, shoes etc.The aim is to provide 2000 education kits annually. It encourages the children to attend school with dignity and also reflects on the increase in school attendance.

				CWF has distributed education kits to 3,500 children from 1st to 7th standard across India.
			</p>
		</div>
		<!-- Scripts -->
			<script src="assets/js/jquery.min.js"></script>
			<script src="assets/js/skel.min.js"></script>
			<script src="assets/js/util.js"></script>
			<script src="assets/js/main.js"></script>
			<script>
				function openTab(tabName) {
  var i, x;
  x = document.getElementsByClassName("containerTab");
  for (i = 0; i < x.length; i++) {
     x[i].style.display = "none";
  }
  document.getElementById(tabName).style.display = "block";
}
			</script>

	</body>
</html>